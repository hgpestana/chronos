...
class EntryForm(ModelForm):
    ...
    """
    task = FormChoiceField(queryset=Task.objects.all().order_by('name'),
                           empty_label=_('Please select an option'), required=False, )
    project = FormChoiceField(queryset=Project.objects.all().order_by('name'),
                              empty_label=_('Please select an option'), required=False, )
    client = FormChoiceField(queryset=Client.objects.all().order_by('name'),
                             empty_label=_('Please select an option'), required=False, )
